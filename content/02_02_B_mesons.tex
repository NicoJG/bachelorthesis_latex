...
The production point, i.e. the collision point of the $pp$ pair, is called the \emph{primary vertex}~(PV) and the decay point is called the \emph{secondary vertex}~(SV).
...
The back-to-back production of the $b\bar{b}$ pair leads to an angular separation of the associated event into two sides.
The \emph{same side}~(SS) contains the decay particles and the fragmentation particles of the signal~$B$.
The \emph{opposite side}~(OS) contains all particles associated with the $b$ quark that is not in the signal~$B$.
