...
Based on the gradient, the weights are adjusted in the opposite direction of the gradient.
The step size at each iteration is determined by an optimizer algorithm.
This thesis used the loss function binary cross entropy in combination with the optimizer Adam.
...
The basis idea of DeepSets is that a function is invariant under permutations of the instances $x$ in a set $X$ if it can be written in the form 
\begin{equation*}
    f(X) = \rho \left( \sum_{x \in X} \phi (x) \right) \, .
\end{equation*}
In DeepSets $\rho$ and $\phi$ are represented by NNs and can be vector-valued \cite{deepset}.
In other words, a NN $\phi$ first extracts some features about each instance in the set.
The feature values of all instances are then summed up and fed into a NN $\rho$ that calculates the desired output.
